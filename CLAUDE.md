...
- ヒット件数を表示
- API は `offset` / `limit` によるページングと任意の `len` / `minLen` 文字数絞り込みに対応し、追加ページの有無を `hasMore` で返す（文字数絞り込みの UI は廃止済み。`len` / `minLen` は API のみの機能）
- 除外文字 `exclude`（任意、最大10文字）：指定したカタカナを含む語を結果から除外する。ひらがな可（クエリと同じ正規化）。SQL の `NOT LIKE` 連結で全検索パスに適用。UI は検索ボックス下の折りたたみ式（設定中はバッジ常時表示、永続化なし）
- 必須文字 `include`（任意、最大10文字）：指定したカタカナを位置を問わず必ず含む語に絞り込む。`LIKE '%X%'` 連結で exclude と同じ型。exclude と同じ文字が指定されたら矛盾として 0 件。UI は除外文字と同じ折りたたみパネル内
- 並び順 `sort=shuffle` + `seed`（任意）：シード付き決定的シャッフル。`ORDER BY (((id + seed) * 1103515245) % 2147483647)` で同一 seed なら全ページ同一順序になり、ページングと両立する。UI は結果ヘッダーの ToggleGroup「五十音/シャッフル」（シャッフル選択中の再タップで引き直し、永続化なし）
- `*` / `＊` を含む検索では、UI から 2〜7 文字・8文字以上の長さで絞り込める
- 0 件時は、条件を緩めた候補が API でヒット確認できた場合のみ「もしかして？」として表示する
- 「まとめてコピー」ボタン：
  - 0 件のときのみ非表示。追加ページあり（`hasMore`）でも表示され、押下で残りページを自動取得してからコピーする
  - 自動取得の上限は 5,000 件（`COPY_FETCH_LIMIT`）。超過時はコピーせず絞り込みを促すトーストを表示
  - 取得中はボタンがスピナー＋進捗件数表示になり、再クリックで中断できる
...
アップロード対象の特定にはデプロイ差分スクリプトを使う（`deploy-*` タグ運用）:
```bash
php scripts/deploy-diff.php          # 前回デプロイ以降の FTP アップロード対象を一覧
php scripts/deploy-diff.php --mark   # アップロード完了後、HEAD にデプロイ済みタグを打つ
```
words.txt / words_addition.txt の変更を検出した場合は words.db の再構築・アップロードも促される。
...
- URL 共有機能
- 五十音ジャンプナビ
- 文字数フィルタ UI（「すべて｜2字｜…｜8字以上」ボタン。2026-06 にユーザー要望で削除。API の `len` / `minLen` パラメータ自体は存続）
- 逆引き検索（漢字・英語表記→カタカナ読み。2026-06 のロードマップ検討時に不採用）
- 本体サイト（crossword-builder.com）からの導線強化（WordPress 側 CTA 改善。2026-06 のロードマップ検討時に不採用）
