...
function updatePatternAssist() {
  ...
  if (!hasStar) {
    ...
    if (chars.every(c => c === '*')) {
      ...
      if (minLen >= 3) parts.push(`${minLen}文字以上`);
    }
    if (digits.length >= 2) {
...
function memKey(query, offset) {
  return `${cacheQueryKey(query)}@${offset}`;
}
...
function cacheRead(query) {
  try {
    const key = 'cw_' + cacheQueryKey(query);
