...
const excludeToggleEl = document.getElementById('exclude-toggle');
...
function currentPatternLabel() {
  ...
  if (selectedLengthFilter > 0) label += `／${selectedLengthFilter}文字`;
  else if (selectedLengthFilter < 0) label += `／${Math.abs(selectedLengthFilter)}文字以上`;
...
if (excludeToggleEl) {
  excludeToggleEl.addEventListener('click', () => {
    const opening = excludePanelEl.hidden;
    excludePanelEl.hidden = !opening;
    updateExcludeUI();
...
function buildLengthFilterControls() {
  if (!lengthFilterEl) return;
  ...
  for (const option of options) {
    ...
    btn.type = 'button';
    btn.className = 'length-filter-btn';
    btn.textContent = option.label;
    btn.dataset.len = String(option.value);
    btn.setAttribute('aria-pressed', option.value === selectedLengthFilter ? 'true' : 'false');
    btn.addEventListener('click', () => {
      const next = Number(btn.dataset.len || '0');
      if (next === selectedLengthFilter) return;
      selectedLengthFilter = next;
      updateLengthFilter();
      restartSearchSoon();
    });
    lengthFilterEl.appendChild(btn);
  }
  updateLengthFilter();
}
...
function updateLengthFilter() {
  ...
  const show = query !== '' && /[*＊]/.test(query);

  if (!show) {
    selectedLengthFilter = 0;
    lengthFilterEl.hidden = true;
  } else {
    lengthFilterEl.hidden = false;
  }

  for (const btn of lengthFilterEl.querySelectorAll('.length-filter-btn')) {
    const active = Number(btn.dataset.len || '0') === selectedLengthFilter;
...
async function doSearch(reset) {
  ...
  // ・パターンのリテラル文字が除外文字と衝突 → マッチする語は存在しない
  if (reset) {
  ...
  const memHit = memCacheGet(query, currentOffset, lengthFilter);
  if (memHit) {
  ...
  if (reset) {
    const cached = cacheRead(query, lengthFilter);
    if (cached) {
      renderResults(cached);
    } else {
      // 楽観サブセット描画：前結果を新クエリでローカルフィルタして仮表示
      // （API 確定まで stale 表示）
      const optimistic = tryOptimisticFilter(query, lengthFilter);
      if (optimistic) {
  ...
  try {
    ...
    // メモリキャッシュへページ単位で保存
    memCacheSet(query, requestOffset, data, lengthFilter);

    if (reset || !currentData) {
      currentData = { ...data };
      renderResults(currentData);
      cacheWrite(query, currentData, lengthFilter); // 1ページ目を sessionStorage にも保存
    } else {
...
function updateCountDisplay(data) {
  ...
  const fullPattern = currentPatternLabel();
...
function renderResults(data) {
  ...
  if (data.count === 0) {
    ...
    if (data.hasMore) {
      ...
      suggestCandidates(currentQuery, selectedLengthFilter); // バックグラウンドで代替候補を取得し、見つかれば表示
    }
    return;
  }
...
function buildCandidates(q) {
  ...
  const chars   = [...q];
  const add = (query, label = query, len = selectedLengthFilter) => {
    out.push({ query, label, len });
  };
  ...
  if (chars.length > 2) {
    add(chars.slice(0, -1).join('')); // 末尾1文字削除
  }
  if (/[1-9１-９]/.test(q)) {
    add(q.replace(/[1-9１-９]/g, '？')); // 数字ワイルドカード → ？
  }

  // 重複除去・元クエリと同一のものを除外・50文字超を除外
  const seen = new Set();
  return out.filter((c) => {
    if (c.query.length === 0 || c.query.length > 50) return false;
    if (c.query === q && c.len === selectedLengthFilter) return false;
    const key = `${c.query}|${c.len}`;
...
async function suggestCandidates(originalQuery, originalLengthFilter) {
  ...
  if (selectedLengthFilter !== originalLengthFilter) return;
...
function renderSuggestions(patterns) {
  ...
  for (const p of patterns) {
    ...
    btn.addEventListener('click', () => {
      selectedLengthFilter = p.len;
      inputEl.value = p.query;
      updateLengthFilter();
      updatePatternAssist();
      inputEl.dispatchEvent(new Event('input', { bubbles: true }));
      restartSearchSoon();
...
function tryOptimisticFilter(newQuery, lengthFilter = 0) {
  ...
  for (const w of currentData.words) {
    ...
    const readingLen = [...normReading].length;
...
function memKey(query, offset, lengthFilter = selectedLengthFilter) {
  return `${cacheQueryKey(query, lengthFilter)}@${offset}`;
}

function memCacheGet(query, offset, lengthFilter = selectedLengthFilter) {
  const k = memKey(query, offset, lengthFilter);
...
function memCacheSet(query, offset, data, lengthFilter = selectedLengthFilter) {
  const k = memKey(query, offset, lengthFilter);
...
function cacheWrite(query, data, lengthFilter = selectedLengthFilter) {
  ...
  try {
    sessionStorage.setItem(
      'cw_' + cacheQueryKey(query, lengthFilter),
      JSON.stringify({ ts: Date.now(), data })
    );
  } catch (_) {} // quota 超過は無視
}

function cacheRead(query, lengthFilter = selectedLengthFilter) {
  try {
    const key = 'cw_' + cacheQueryKey(query, lengthFilter);
