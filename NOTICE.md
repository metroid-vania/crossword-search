...
4. **検索用正規化列の生成**：拗音・促音を大文字相当に展開した `normalized` と、
   検索用文字数 `len` を生成
5. **短すぎる語の除外**：2 文字未満の見出し語を除外
6. **表記バリエーションの制限**：`/` 区切りの表記候補を先頭 10 件までに制限
7. **注釈の除去**：`;` 以降の注釈・語義コメントを削除
8. **フォーマット変換**：SKK 形式 → `見出し /表記1/表記2/.../` 形式の `words.txt`、
   さらに SQLite データベース `words.db`（テーブル `words(id, reading, normalized, len, variants)`）
   へ変換
9. **独自追加語の統合**：`words_addition.txt`（本アプリのために新規作成、同じく GPL v2 以降）
   のエントリを合成
