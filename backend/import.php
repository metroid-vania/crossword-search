<?php
/**
 * words.txt / words_addition.txt を SQLite データベース（words.db）に取り込むスクリプト
 * CLI から実行: php import.php
 *
 * --------------------------------------------------------------------
 * 辞書データの由来とライセンス
 * --------------------------------------------------------------------
 *  - words.txt は SKK-JISYO.L（https://github.com/skk-dev/dict）から派生した
 *    辞書データです。SKK-JISYO.L は GPL v2 or later でライセンスされています。
 *  - 本スクリプトは SKK-JISYO.L に対する「改変に適した形式（preferred form
 *    for modification）」の一部として機能し、以下の加工を行います：
 *      1. エンコーディング UTF-8 化（前処理段階）
 *      2. 見出し語のうち 2〜13 文字のみを採用
 *      3. ひらがな → カタカナ正規化
 *      4. 拗音・促音の検索用正規化（ァィゥェォッャュョ → ア行・ツ・ヤ行）
 *      5. 表記バリエーションを先頭 10 件までに制限
 *      6. SQLite テーブル words(id, reading, normalized, len, variants) へ変換
 *  - 原著作権表示および本アプリによる改変内容の詳細は、リポジトリルートの
 *    NOTICE.md を参照してください。
 * --------------------------------------------------------------------
 */
